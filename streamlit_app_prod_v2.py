...
@st.cache_resource
def load_pretrained(tfidf_url, svd_url, scaler_url, model_download_url, model_local_path):
    tfidf = jl.load(tfidf_url)
    svd = jl.load(svd_url)
    scaler = jl.load(scaler_url)

    # Download the model if it hasn't already been.
    ...
    model = jl.load(model_local_path)

    return tfidf, svd, scaler, model

tfidf, svd, scaler, model = load_pretrained(tfidf_url, svd_url, scaler_url, model_download_url, model_local_path)
